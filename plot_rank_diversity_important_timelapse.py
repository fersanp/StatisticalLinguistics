...
for n in range(1,7):
    for timelapse in lapse:
        ...
        for journal in os.listdir('results_rank_important/'):
            if journal == ".DS_Store":
                continue

#            df=pd.read_csv('results_rank/'+journal+"/"+timelapse+"/"+str(n)+'grams_RD.txt',sep='\t',names=['rank','diversity'])
            diversity = np.loadtxt('results_rank_important/'+journal+"/"+timelapse+"/"+str(n)+'grams_RD.txt')
...
#        plt.savefig('graphs_important/physical_rank_diversities_'+timelapse+"_fit_witherrors"+str(n)+'grams'+'.png')#,dpi=256)
        ...
        if not os.path.exists(outpath):
            os.makedirs(outpath)
        plt.savefig(outpath+'/physical_rank_diversities_important_'+timelapse+"_"+str(n)+'grams'+'.png')#,dpi=256)
